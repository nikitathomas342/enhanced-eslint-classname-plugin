...
const rule = {
  ...
  create(context) {
        ...
        switch (node.value?.type) {
          ...
          case "JSXExpressionContainer": {
            ...
            if (
              ...
              if (args.length > maxSpaceSeparatedClasses) return;
              if (
                !args.every(
                  /**
              ...
              if (args.some((arg) => /\s+/g.test(arg.value))) return;
    ...
    function suggestToUseFunctionIfViolated(node, classString) {
      ...
      if (classes.length <= maxSpaceSeparatedClasses) return;
        ...
        return fixer.replaceText(
          ...
          `className={${functionName}(${classes
            .reduce((prev, curr, index) => {
              if (index % maxSpaceSeparatedClasses === 0 || index === 0) {
                // @ts-ignore
                prev.push([curr]);
              } else {
                // @ts-ignore
            ...
            .reduce((prev, curr) => {
              // @ts-ignore
              prev.push(JSON.stringify(curr.join(" ")));
              return prev;
            }, [])
            .join(", ")})}`
        );
    ...
    function suggestToAvoidFunction(node, classes) {
      ...
      context.report({
        ...
        suggest: [{ messageId: "suggestToAvoid", data: { functionName }, fix }],
        fix,
      });
    }
  },
